...
namespace Dan200.Game.Components.Editor
{
    ...
    internal enum TransformManipulationMode
    {
        Position,
        Rotation,
    }

    internal struct TransformManipulatorComponentData
    {
    }
    ...
    internal class TransformManipulatorComponent : Component<TransformManipulatorComponentData>, IManipulator
    {
        ...
        public TransformManipulationMode Mode
        {
            get;
            set;
        }
        ...
        protected override void OnInit(in TransformManipulatorComponentData properties)
        {
            m_gui = Level.GetSystem<GUISystem>();
            m_transform = Entity.GetComponent<TransformComponent>();
            m_editor = Entity.GetComponent<EditorComponent>();

            Mode = TransformManipulationMode.Position;
        ...
        {
            // Determine how to orient the manipulator
            ...
            // Do translation
            if (Mode == TransformManipulationMode.Position && PositionProperty != null)
            {
            ...
            // Do rotation
            if (Mode == TransformManipulationMode.Rotation && RotationProperty != null)
            {
                // TODO!
            }
        ...
        private void Move(Vector3 delta)
        {
            ...
            ApplyTransform(transform, true, false);
        }

        private void ApplyTransform(in Matrix4 transform, bool position, bool rotation)
        {
            m_transform.Transform = transform;
            if (position)
            {
                App.Assert(PositionProperty != null);
                m_editor.Properties[PositionProperty] = m_transform.LocalPosition.ToLuaValue();
            }
            if (rotation)
            {
                App.Assert(RotationProperty != null);
                var angles = m_transform.LocalTransform.GetRotationAngles() * Mathf.RADIANS_TO_DEGREES;
                m_editor.Properties[RotationProperty] = angles.ToLuaValue();
            }
            m_editor.ResetFromProperties();
        }
    }
}
