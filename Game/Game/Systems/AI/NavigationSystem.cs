...
namespace Dan200.Game.Systems.AI
{
    ...
    internal struct NavigationSystemData
    {
    }

    [RequireSystem(typeof(PhysicsSystem))]
    internal class NavigationSystem : System<NavigationSystemData>, IUpdate, IDebugDraw
    {
        ...
        protected override void OnInit(in NavigationSystemData properties)
        {
        ...
        private bool CheckLineOfSight(NavGraph.Node origin, NavGraph.Node destination, out float o_cost)
        {
            ...
            if(!m_physics.World.SphereCast(ray, 1.0f, CollisionGroup.Environment, out result))
            {
