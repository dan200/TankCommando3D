...
{
    internal class SteamworksWorkshop : IWorkshop
    {
        ...
        {
            var result = new Promise<WorkshopPublishResult>();
            m_network.MakeCall(
                ...
                delegate (CreateItemResult_t args, bool ioFailure)
                {
                    if (ioFailure || args.m_eResult != EResult.k_EResultOK)
                    {
                        if (!ioFailure)
                        {
                            result.Fail(new SteamworksException("Failed to create file: " + args.m_eResult));
                        }
                        else
                        {
                            result.Fail(new SteamworksException("Failed to create file"));
                        }
                    }
                    else
                    {
        ...
        private void UpdateItem(ulong itemID, string changeMessage, string filePath, string previewImagePath, string title, string description, string[] tags, bool? visibility, Promise<WorkshopPublishResult> promise)
        {
            ...
            m_network.MakeCall(
                ...
                delegate (SubmitItemUpdateResult_t args, bool ioFailure)
                {
                    if (ioFailure || args.m_eResult != EResult.k_EResultOK)
                    {
                        if (!ioFailure)
                        {
                            promise.Fail(new SteamworksException("Failed to update file: " + args.m_eResult));
                        }
                        else
                        {
                            promise.Fail(new SteamworksException("Failed to update file"));
                        }
                    }
                    else
                    {
            ...
            m_network.MakeCall(
                ...
                delegate (RemoteStorageSubscribePublishedFileResult_t args, bool ioFailure)
                {
                    if (ioFailure || args.m_eResult != EResult.k_EResultOK)
                    {
                        if (!ioFailure)
                        {
                            promise.Fail(new SteamworksException("Failed to subscribe to item: " + args.m_eResult));
                        }
                        else
                        {
                            promise.Fail(new SteamworksException("Failed to subscribe to item"));
                        }
                    }
                    else
                    {
            ...
            m_network.MakeCall(
                ...
                delegate (RemoteStorageUnsubscribePublishedFileResult_t args, bool ioFailure)
                {
                    if (ioFailure || args.m_eResult != EResult.k_EResultOK)
                    {
                        if (!ioFailure)
                        {
                            promise.Fail(new SteamworksException("Failed to unsubscribe from item: " + args.m_eResult));
                        }
                        else
                        {
                            promise.Fail(new SteamworksException("Failed to unsubscribe from item"));
                        }
                    }
                    else
                    {
        ...
        {
            var result = new Promise<WorkshopItemInfo[]>();
            ...
            m_network.MakeCall(
                ...
                delegate (SteamUGCQueryCompleted_t args, bool ioFailure)
                {
                    if (ioFailure || args.m_eResult != EResult.k_EResultOK)
                    {
                        if (!ioFailure)
                        {
                            result.Fail(new SteamworksException("Failed to get item info: " + args.m_eResult));
                        }
                        else
                        {
                            result.Fail(new SteamworksException("Failed to get item info"));
                        }
                    }
                    else
                    {
                        ...
                        for (uint i = 0; i < args.m_unNumResultsReturned; ++i)
                        {
							...
							if (SteamUGC.GetQueryUGCResult(args.m_handle, i, out details) &&
						    	SteamUGC.GetQueryUGCStatistic(args.m_handle, i, EItemStatistic.k_EItemStatistic_NumSubscriptions, out subscribers) &&
						    	SteamUGC.GetQueryUGCStatistic(args.m_handle, i, EItemStatistic.k_EItemStatistic_NumUniqueSubscriptions, out totalSubscribers) &&
						        !details.m_bBanned)
                            {
                                ...
                                info.TotalSubscribers = (int)System.Math.Max(subscribers, totalSubscribers);
					...
					SteamUGC.ReleaseQueryUGCRequest(args.m_handle);
                }
            );
            ...
            m_network.MakeCall(
                ...
                delegate (SetUserItemVoteResult_t args, bool ioFailure)
                {
                    if (ioFailure || args.m_eResult != EResult.k_EResultOK)
                    {
                        if (!ioFailure)
                        {
                            result.Fail(new SteamworksException("Failed to submit item vote: " + args.m_eResult));
                        }
                        else
                        {
                            result.Fail(new SteamworksException("Failed to submit item vote"));
                        }
                    }
                    else
                    {
            ...
            m_network.MakeCall(
                ...
                delegate (RemoteStorageSetUserPublishedFileActionResult_t args, bool ioFailure)
                {
                    if (ioFailure || args.m_eResult != EResult.k_EResultOK)
                    {
                        if (!ioFailure)
                        {
                            result.Fail(new SteamworksException("Failed to set item played: " + args.m_eResult));
                        }
                        else
                        {
                            result.Fail(new SteamworksException("Failed to set item played"));
                        }
                    }
                    else
                    {
            ...
            m_network.MakeCall(
                ...
                delegate (RemoteStorageSetUserPublishedFileActionResult_t args, bool ioFailure)
                {
                    if (ioFailure || args.m_eResult != EResult.k_EResultOK)
                    {
                        if (!ioFailure)
                        {
                            result.Fail(new SteamworksException("Failed to set item completed: " + args.m_eResult));
                        }
                        else
                        {
                            result.Fail(new SteamworksException("Failed to set item completed"));
                        }
                    }
                    else
                    {
