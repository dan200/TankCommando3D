    ...
    internal class Mods
    {
        ...
        public static IEnumerable<Mod> RemovedLoadedMods
        {
            get
            {
                for (int i = s_removedLoadedMods.Count - 1; i >= 0; --i)
                {
                    ...
                    {
                        s_removedLoadedMods.UnorderedRemoveAt(i);
                    }
                }
            }
        }
        ...
        private static bool TryAddMod(string path, ModSource source, out Mod o_mod)
        {
            ...
            catch (Exception)
            {
                ...
                {
                    App.LogError("Error loading mod {0}. Ignoring", path);
                    s_ignoredMods.Add(path);
                    changed = true;
                }
                if (s_availableMods.ContainsKey(path))
                {
                    App.LogError("Error loading mod {0}. Disabling", path);
                    mod = s_availableMods[path];
            ...
            if (App.Info.Version < mod.MinimumGameVersion)
            {
                ...
                if (!s_ignoredMods.Contains(path))
                {
                    App.LogError("Ignoring mod {0} ({1}): Requires game version {2}", mod.Title, mod.Path, mod.MinimumGameVersion);
                    s_ignoredMods.Add(path);
                    changed = true;
                }
                if (s_availableMods.ContainsKey(path))
                {
                    App.LogError("Disabling mod {0} ({1}): Requires game version {2}", mod.Title, mod.Path, mod.MinimumGameVersion);
                    s_availableMods.Remove(path);
                    if (mod.Loaded)
                    {
                        s_removedLoadedMods.Add(mod);
        ...
        private static bool TryRemoveMod(string path)
        {
            bool changed = false;
            Mod mod;
            if (s_availableMods.TryGetValue(path, out mod))
            {
...
#if ZIP
                        || (File.Exists(path) && path.EndsWith(".zip"))
#endif
                        )
                    {
                        Mod mod;
        ...
        public static Mod Create(string title)
        {
			...
			table["Title"] = title;
			table["Version"] = new Version(1, 0, 0).ToString();
			table["GameVersion"] = App.Info.Version.ToString();
			table["AutoLoad"] = true;

            var infoPath = Path.Combine(path, "info.txt");
			using (var stream = File.Open(infoPath, FileMode.Create))
            {
				var lon = new LONEncoder(stream);
				lon.EncodeComment("Mod Information");
				lon.Encode(table);
            }
